...
interface IGovernedModuleRegistry {
    ...
    /// @param moduleKey Stable module key.
    ...
    /// @dev Must reject zero and duplicate addresses; registration does not grant execution authority.
    /// @param moduleKey Stable module key.
    ...
    /// @dev Removal uses an O(n) internal scan and reverts for unknown keys without mutating unrelated entries.
    /// @param moduleKey Stable module key to remove.
    ...
    /// @notice Reads a module by stable key.
    /// @param moduleKey Stable module key.
